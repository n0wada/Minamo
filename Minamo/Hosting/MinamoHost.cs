...
public enum MinamoCapabilityMode
{
    Automatic,
    Restricted,
    Unrestricted
...
public sealed class MinamoHostOptions
{
...
public sealed class MinamoHost
{
    private readonly object programOwner = new();
    private readonly Dictionary<string, MinamoModuleBuilder> modules =
    ...
    public MinamoHost(MinamoHostOptions? options = null)
    {
        options ??= new();
    ...
    public IReadOnlyCollection<MinamoModuleBuilder> Modules => modules.Values;

    [EditorBrowsable(EditorBrowsableState.Never)]
    public MinamoHost UseFileLookup(FileLookup fileLookup)
    {
    ...
    public MinamoHost DisableFileImports()
    {
        lookup = null;
    ...
    public MinamoHost AddCapabilities(params string[] capabilities)
    {
    ...
    public MinamoHost AddResourceType<T>() where T : MinamoResource
    {
        ...
        var definition = MinamoResourceDefinition.Create<T>();
    ...
    public MinamoHost Module(string name, Action<MinamoModuleBuilder> configure)
    {
        ...
        var module = new MinamoModuleBuilder(name);
        configure(module);
        modules.Add(name, module);
        return this;
    }

    public MinamoInstance CreateInstance(object? hostContext = null) =>
        CreateInstance(new MinamoEnvironment(hostContext));

    public MinamoInstance CreateInstance(MinamoEnvironment environment) =>
        CreateInstance(environment, null);

    public MinamoInstance CreateInstance(MinamoProgram program, MinamoEnvironment? environment = null)
    {
        ArgumentNullException.ThrowIfNull(program);
        if (!ReferenceEquals(program.Owner, programOwner))
        {
            throw new InvalidOperationException(
                "A compiled Minamo program can only be used with the host that compiled it.");
        }

        return CreateInstance(environment ?? new MinamoEnvironment(), program);
    }

    internal MinamoInstance CreateInstance(object? hostContext, MinamoTuple? arguments) =>
        CreateInstance(new MinamoEnvironment(hostContext), null, arguments);

    internal MinamoInstance CreateInstance(MinamoEnvironment environment, MinamoTuple? arguments) =>
        CreateInstance(environment, null, arguments);

    private MinamoInstance CreateInstance(
        MinamoEnvironment environment,
        MinamoProgram? program,
        MinamoTuple? arguments = null)
    {
        ...
        var hostEnvironment = new MinamoHostEnvironment(
            ...
            {
                MinamoCapabilityMode.Automatic => capabilities.Count == 0,
                MinamoCapabilityMode.Restricted => false,
                MinamoCapabilityMode.Unrestricted => true,
    ...
    public Result<MinamoProgram> Compile(string source, string? sourceName = null)
    {
        ArgumentNullException.ThrowIfNull(source);
        return Compile(SourceBuffer.FromString(source, sourceName));
    }

    public Result<MinamoProgram> CompileFile(string fileName)
    {
    ...
    public Result<MinamoProgram> Compile(SourceBuffer source)
    {
        ArgumentNullException.ThrowIfNull(source);
        var definitions = modules.Values.Select(m => m.Build()).ToArray();
        ...
        var linker = new MinamoLinker(compileLookup);
        var result = linker.Make(source);
        return result.Success && result.Value is not null
            ? Result.Create(new MinamoProgram(result.Value, result.Messages, programOwner), result.Messages)
            : Result.Create<MinamoProgram>(null, result.Messages);
    }
...
internal static class HostNames
{
    public static void ValidateIdentifier(string name, string parameterName, string kind)
    {
        ...
        if (!IsIdentifier(name))
        {
            throw new ArgumentException(
                $"Host {kind} name '{name}' is not a valid Minamo identifier.",
                parameterName);
        }
    }
    ...
    public static void ValidateDottedName(string name, string parameterName, string kind)
    {
        ...
        foreach (var segment in name.Split('.'))
        {
            ...
            {
                throw new ArgumentException(
                    $"Host {kind} name '{name}' must contain only Minamo identifier segments.",
                    parameterName);
            }
        }
    }
...
internal static class MinamoResourceDefinition
{
    internal static HostResourceDefinition Create<T>() where T : MinamoResource
    {
        var type = typeof(T);
        if (type.IsAbstract)
        {
        ...
        var resource = type.GetCustomAttribute<MinamoResourceAttribute>(inherit: false)
            ?? throw new InvalidOperationException(
                $"Resource type '{type.FullName}' requires MinamoResourceAttribute.");
        ...
        var methods = type
            ...
            .Select(method => (Method: method, Command:
                method.GetCustomAttribute<MinamoCommandAttribute>(inherit: false)))
    ...
    private static ResourceMethod CreateCommand(
        MethodInfo method,
        MinamoCommandAttribute command)
    {
        ...
        var contextCount = 0;
        var exposed = new List<MinamoCommandParameter>();
        foreach (var parameter in method.GetParameters())
        {
            ...
            if (parameter.ParameterType == typeof(MinamoCommandContext))
            {
                contextCount++;
    ...
    {
        internal MinamoCommandDescriptor CatalogDescriptor() =>
            new(
                Name,
                Description,
                Capability,
                Parameters,
                (MinamoCommandHandler)(_ => MinamoNil.Instance));

        internal MinamoCommandDescriptor Bind(MinamoResource resource) =>
            new(
                Name,
                Description,
                Capability,
        ...
        private MinamoObject Invoke(MinamoResource resource, MinamoCommandContext context)
        {
            ...
            {
                arguments[i] = parameters[i].ParameterType == typeof(MinamoCommandContext)
                    ? context
            ...
            if (result is Task task)
            {
                return Method.ReturnType.IsGenericType
                    ? MinamoCommandConvert.FromAwaitable(
                        task,
                        Method.ReturnType.GetGenericArguments()[0])
                    : MinamoCommandConvert.FromAwaitable(task);
            }
            if (Method.ReturnType == typeof(ValueTask))
            {
                return MinamoCommandConvert.FromAwaitable((ValueTask)result!);
            }
            if (Method.ReturnType.IsGenericType
                && Method.ReturnType.GetGenericTypeDefinition() == typeof(ValueTask<>))
            {
                var asTask = Method.ReturnType.GetMethod(nameof(ValueTask<int>.AsTask))!.Invoke(result, null)!;
                return MinamoCommandConvert.FromAwaitable(
                    (Task)asTask,
                    Method.ReturnType.GetGenericArguments()[0]);
            }

            return MinamoCommandConvert.FromObject(result, Method.ReturnType);
        }
    }
}
