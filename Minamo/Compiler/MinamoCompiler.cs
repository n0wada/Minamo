...
public static class MinamoCompiler
{
        ...
        string? sourceName = null)
    {
        ...
        return new MinamoLinker(lookup).Make(SourceBuffer.FromString(source, sourceName));
    }

    public static Result<UnitComposition> Compile(
        MinamoCodeModel codeModel,
        BuilderOptions? options = null)
    {
        ...
        FileLookup lookup)
    {
        ArgumentNullException.ThrowIfNull(codeModel);
        ArgumentNullException.ThrowIfNull(lookup);
        return new MinamoLinker(lookup).Make(codeModel);
    }
    ...
    public static Result<UnitComposition> CompileFile(string path, FileLookup lookup)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(path);
        ArgumentNullException.ThrowIfNull(lookup);
        return new MinamoLinker(lookup).Make(SourceBuffer.FromFile(path));
    }
}
