...
internal sealed class StackMachineEmitter
{
    ...
    private readonly Dictionary<MinamoObject, int> objects;
    ...
    private int IndexObject(MinamoObject val)
    {
    ...
    public void Push(string val) => Push(new MinamoString(val));

    public void Push(char val) => Push(new MinamoChar(val));

    public void Push(MinamoObject val) => Emit(new(OpCode.LoadConst, IndexObject(val)));
    ...
    public void CheckContains(string field) => Emit(new(OpCode.CheckContains, IndexObject(new MinamoString(field))));
    ...
    public void CreateSelectFactory(MinamoObject definition, int closureCount) =>
        Emit(new(OpCode.CreateSelectFactory, IndexObject(definition), closureCount), -closureCount + 1);
    public void CreateSelectFactoryTemplate(MinamoObject name) =>
