...
using Minamo.Compiler.Lowering;
...
internal sealed partial class Builder : ILoweredEmitterTarget
{
    ...
    private readonly MinamoLinker linker; //Linker
    ...
    public Builder(BuilderOptions options, MinamoLinker linker)
    {
        referencedUnits = new();
        importedSymbols = new();
    ...
    public Unit? Build(MinamoCodeModel codeModel)
    {
        diagnostics.Clear();
    ...
    private bool TryBuild(MinamoCodeModel codeModel)
    {
        try
        {
    ...
    private MinamoBuildException Ice(Exception? ex = null) => new($"Internal compiler error: {ex?.Message}", ex);
    ...
    void ILoweredEmitterTarget.ThrowError(MinamoError code) =>
        ThrowError(code);
...
partial class Builder
{
    ...
    private void ThrowErrorProlog(MinamoError code, int parameters)
    {
        cw.LoadType(MinamoTypeCodes.Exception);
        cw.LoadMember(code.ToString());
        cw.PrepareCall(parameters);
    }

    private void ThrowError(MinamoError code)
    {
