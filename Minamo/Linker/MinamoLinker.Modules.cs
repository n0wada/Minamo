...
public partial class MinamoLinker
{
    ...
    private MinamoCodeModel? ProcessBuffer(SourceBuffer buffer)
    {
        var res = MinamoParser.Parse(buffer);
    ...
    protected virtual Unit? CompileNodes(MinamoCodeModel codeModel, bool root)
    {
        var compiler = new MinamoCompilerEngine(BuilderOptions, this);
    ...
    private bool FindModuleExact(string workingDir, string module, Reference mod, out string? path)
    {
        ...
        {
            var sf = Path.Combine(Path.GetDirectoryName(fullPath)!, Path.GetFileNameWithoutExtension(fullPath) + ".nami");
