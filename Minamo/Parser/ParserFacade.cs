...
public static class MinamoParser
{
    private const string DefaultBuffer = "<stdin>";

    public static Result<MinamoCodeModel> Parse(string source, string? sourceName = null)
    {
        ArgumentNullException.ThrowIfNull(source);
        return Parse(SourceBuffer.FromString(source, sourceName));
    }

    public static Result<MinamoCodeModel> ParseFile(string path)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(path);
        return Parse(SourceBuffer.FromFile(path));
    }

    public static Result<MinamoCodeModel> Parse(SourceBuffer buffer)
    {
...
internal static class ParserSelfTest
{
    ...
    private static Result<MinamoCodeModel> Parse(string source) => CreateParser(source).Parse();
