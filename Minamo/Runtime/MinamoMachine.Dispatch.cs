...
internal static partial class MinamoMachine
{
    ...
    private static VmDispatchResult ExecuteLoadStore(Op op, VmState state)
    {
        switch (op.Code)
        {
            ...
            case OpCode.LoadConst:
                ...
                break;
            case OpCode.LoadNil:
                state.EvalStack.Push(MinamoNil.Instance);
                break;
            ...
            case OpCode.LoadInt0:
                state.EvalStack.Push(MinamoInteger.Zero);
                break;
            case OpCode.LoadInt1:
                state.EvalStack.Push(MinamoInteger.One);
                break;
            case OpCode.LoadFloat0:
                state.EvalStack.Push(MinamoFloat.Zero);
                break;
            case OpCode.LoadFloat1:
                state.EvalStack.Push(MinamoFloat.One);
                break;
            ...
            case OpCode.LoadExternal:
                ...
                if (state.Second is MinamoFunction function && function.Auto)
                {
                    state.Second = function.TryInvokeProperty(state.Context, MinamoNil.Instance);
                }
            ...
            case OpCode.LoadEnvironment:
                ...
                var environment = state.Context.GetContextVariable<MinamoEnvironment>(
                    MinamoEnvironment.ContextKey);
    ...
    private static VmDispatchResult ExecuteBinary(Op op, VmState state) =>
        MinamoMachineHelpers.ExecuteBinaryOperation(
    ...
    {
        state.Second = null;
        return MinamoMachineHelpers.ExecuteUnaryOperation(op.Code, state.EvalStack, state.Types, state.Context)
            ? VmDispatchResult.Continue
            : new(VmStep.HandleCallback);
    }
    ...
    private static VmDispatchResult ExecuteFunction(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateFunction:
                state.EvalStack.Push(MinamoMachineHelpers.ExecuteFunctionCreation(
            ...
            case OpCode.SetFunctionAttribute:
                ((MinamoFunction)state.EvalStack.Peek()).Attr |= op.Data;
                break;
            case OpCode.CreateSelectFactory:
                var definition = state.Unit.Objects[op.Data] as MinamoSelectDefinitionValue
                    ?? throw new InvalidOperationException("The select definition is unavailable.");
                var closures = new MinamoFunction[op.Data2];
                for (var i = closures.Length - 1; i >= 0; i--)
                {
                    closures[i] = state.EvalStack.Pop() as MinamoFunction
                        ?? throw new InvalidOperationException("A select closure is unavailable.");
                }
                state.EvalStack.Push(new MinamoSelectFactory(definition, closures));
                break;
            case OpCode.CreateSelectFactoryTemplate:
                var initializer = state.EvalStack.Pop() as MinamoFunction
                    ?? throw new InvalidOperationException("A select factory initializer is unavailable.");
                var name = state.Unit.Objects[op.Data] as MinamoString
                    ?? throw new InvalidOperationException("The select factory name is unavailable.");
                state.EvalStack.Push(new MinamoSelectFactory(name.Value, initializer));
                break;
            default:
                throw UnexpectedOpcode(op);
        }
    ...
    private static VmDispatchResult ExecuteMember(Op op, VmState state)
    {
        switch (op.Code)
        {
            ...
            case OpCode.LoadMember:
                return MinamoMachineHelpers.ExecuteNamedMemberAccess(
            ...
            case OpCode.StoreMember:
                return MinamoMachineHelpers.ExecuteTypeMemberMutation(
    ...
    private static VmDispatchResult ExecuteIndex(Op op, VmState state)
    {
        ...
        {
            OpCode.LoadIndex => MinamoMachineHelpers.ExecuteIndexedAccess(
                state.EvalStack, state.Types, state.Context, out state.First, out state.Second),
            OpCode.LoadRawIndex => MinamoMachineHelpers.ExecuteRawIndexedAccess(
                state.EvalStack, state.Types, state.Context, out state.First, out state.Second),
            OpCode.StoreIndex => MinamoMachineHelpers.ExecuteIndexedSet(
                state.EvalStack, state.Types, state.Context,
                out state.First, out state.Second, out state.Third),
            OpCode.StoreRawIndex => MinamoMachineHelpers.ExecuteRawIndexedSet(
                state.EvalStack, state.Types, state.Context,
                out state.First, out state.Second, out state.Third),
    ...
    private static VmDispatchResult ExecuteModuleInstruction(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.LoadModule:
                ...
                state.EvalStack.Push(new MinamoModule(
            ...
            case OpCode.FinishModule:
                ...
                {
                    throw new MinamoRuntimeException(MessageCatalog.Get(MessageGroup.Runtime, "StackCorrupted.0"));
                }
    ...
    private static VmDispatchResult ExecuteIterator(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateIterator:
                state.EvalStack.Push(MinamoMachineHelpers.ExecuteIteratorCreation(
                    state.Function, op.Data, state.Locals));
                break;
            case OpCode.LoadTerminator:
                state.EvalStack.Push(MinamoNil.Terminator);
                break;
            case OpCode.Yield:
            ...
            case OpCode.JumpIfTerminator:
                if (ReferenceEquals(state.EvalStack.Peek(), MinamoNil.Terminator))
                {
                    state.Offset = op.Data;
                }
            ...
            case OpCode.JumpIfIteratorValue:
                if (state.EvalStack.Peek().TypeId == MinamoTypeCodes.Iterator)
                {
                    state.Offset = op.Data;
                }
            ...
            case OpCode.LoadIteratorFunction:
                ...
                if (state.Second is MinamoIterator iterator)
                {
                    state.EvalStack.Replace(iterator.GetIteratorFunction());
                }
                else
                {
                    state.Context.InvalidType(MinamoTypeCodes.Iterator, state.Second);
                    return new(VmStep.Throw);
                }
                break;
            default:
    ...
    private static VmDispatchResult ExecuteCall(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.PrepareCall:
                return MinamoMachineHelpers.TryPrepareFunction(
                    state.EvalStack, state.Types, state.Context, op.Data, out _)
                    ? VmDispatchResult.Continue
                    : new(VmStep.Throw);
            case OpCode.SetCallArgument:
                MinamoMachineHelpers.AssignIndexedArgument(
            ...
            case OpCode.SetNamedCallArgument:
                ...
                var function = (MinamoFunction)state.EvalStack.Peek(2);
                return MinamoMachineHelpers.TryAssignNamedArgument(
            ...
            case OpCode.TailCall:
                ...
                MinamoMachineHelpers.PrepareStandardCall(
            ...
            case OpCode.Call:
                ...
                if (MinamoMachineHelpers.ExecutePositionalFunctionCall(
                ...
                {
                    return new(VmStep.Suspend, Suspension: new(awaitable));
                }
            ...
            case OpCode.CallStatic:
                if (MinamoMachineHelpers.ExecuteNamedMemberCall(
                    op.Code, state.EvalStack, state.Types, state.Unit.Strings[op.Data],
            ...
            case OpCode.InvokePreparedCall:
                if (MinamoMachineHelpers.ExecuteFunctionCall(
                    state.EvalStack, state.Context, state.Function, state.Offset,
    ...
    private static VmDispatchResult ExecuteCollection(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateLabel:
                state.EvalStack.Replace(new MinamoLabel(
                    (string)state.Unit.Strings[op.Data], state.EvalStack.Peek()));
                return VmDispatchResult.Continue;
            case OpCode.CreateArguments:
                state.EvalStack.Push(op.Data == 0
                    ? MinamoTuple.Empty
                    : MinamoMachineHelpers.MakeTuple(state.EvalStack, op.Data, true));
                return VmDispatchResult.Continue;
            case OpCode.CreateTuple:
                state.EvalStack.Push(op.Data == 0
                    ? MinamoTuple.Empty
                    : MinamoMachineHelpers.MakeTuple(state.EvalStack, op.Data, false));
                return VmDispatchResult.Continue;
            case OpCode.CreateDictionary:
                state.EvalStack.Push(MinamoMachineHelpers.MakeDictionary(state.EvalStack, op.Data));
                return VmDispatchResult.Continue;
            case OpCode.CheckContains:
                state.Second = state.Unit.Objects[op.Data];
                return MinamoMachineHelpers.ExecuteContains(
    ...
    private static VmDispatchResult ExecuteConversion(Op op, VmState state)
    {
        ...
        state.Second = null;
        return MinamoMachineHelpers.ExecuteStringConversion(
            state.EvalStack, state.Types, state.Context)
            ? VmDispatchResult.Continue
            : new(VmStep.HandleCallback);
    }
    ...
    private static VmDispatchResult ExecuteMetadata(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.AddTypeAnnotation:
                ...
                state.Second = state.EvalStack.Peek();
                ((MinamoLabel)state.Second).AddTypeAnnotation((MinamoTypeInfo)state.First);
                break;
            case OpCode.MarkMutable:
                ((MinamoLabel)state.EvalStack.Peek()).Mutable = true;
                break;
            default:
                throw UnexpectedOpcode(op);
        }
    ...
    private static VmDispatchResult ExecuteCast(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateCast:
                MinamoMachineHelpers.ExecuteCastRegistration(
                    state.EvalStack, out state.First, out state.Second);
                return VmDispatchResult.Continue;
            case OpCode.ApplyCast:
                return MinamoMachineHelpers.ExecuteCast(
                    state.EvalStack, state.Types, state.Context,
                    out state.First, out state.Second)
                    ? VmDispatchResult.Continue
                    : new(VmStep.Throw);
