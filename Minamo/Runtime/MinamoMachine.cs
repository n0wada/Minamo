...
internal static partial class MinamoMachine
{
    ...
    private static MinamoNativeFunction Global(int unitId) => new(null, unitId, 0, FastList<MinamoObject[]>.Empty, -1);
    ...
    private static ExecutionResult ExecuteModule(int unitId, ExecutionContext ctx)
    {
        ...
        if (unit.Layouts.Count == 0) //This is a foreign module
        {
            ...
            return ExecutionResult.Fetch(0, MinamoNil.Instance, ctx);
        }
        ...
        {
            var mems = new MinamoObject[lay0.Size];
        ...
        ctx.RuntimeContext.Units[unitId] = ctx.RuntimeContext.Units[unitId] ?? new MinamoObject[lay0.Size];
        return ExecuteWithDataResult(Global(unitId), Array.Empty<MinamoObject>(), ctx);
    }

    internal static MinamoObject ExecuteWithData(MinamoNativeFunction function, MinamoObject[] locals, ExecutionContext ctx)
    {
        ...
        return result.Value ?? MinamoNil.Instance;
    }

    internal static ExecutionResult ExecuteWithArguments(
        MinamoNativeFunction function,
        ...
        if (ctx.CallCnt > maxCallDepth)
        {
            if (ctx.Control?.MaxCallDepth is not null)
            {
                throw new MinamoExecutionLimitException(
                    MinamoExecutionLimitKind.CallDepth,
                    $"Call depth limit of {maxCallDepth} was exceeded.");
            }

            throw new MinamoRuntimeException(MessageCatalog.Get(MessageGroup.Runtime, "StackOverflow.0"));
        }
    ...
    {
        public VmState(MinamoNativeFunction function, MinamoObject[] locals, ExecutionContext context)
        {
            Function = function;
            Locals = locals;
            Context = context;
        ...
        public FastList<MinamoTypeInfo> Types { get; }
        public MinamoNativeFunction Function;
        public MinamoObject[] Locals;
        public Unit Unit;
        public FastList<Op> Ops;
        public int Offset;
        public EvalStack EvalStack;
        public MinamoObject? First;
        public MinamoObject? Second;
        public MinamoObject? Third;
    }
    ...
    private static void EnterFunction(MinamoNativeFunction function, ref MinamoObject[] locals, ExecutionContext ctx,
    ...
    private static void LoadProgram(MinamoNativeFunction function, ExecutionContext ctx, out Unit unit, out FastList<Op> ops)
    {
    ...
    private static bool ResumeCallerOrFinish(ExecutionContext ctx, EvalStack currentStack,
        ref MinamoNativeFunction function, ref MinamoObject[] locals, ref int offset, ref EvalStack evalStack,
        out MinamoObject result)
    {
        if (MinamoMachineHelpers.TryResumeCaller(ctx, currentStack, out result, out function, out locals, out offset, out evalStack))
        {
            return true;
        }
    ...
    internal static void FillDefaults(MinamoObject[] locals, MinamoFunction callFun, ExecutionContext ctx) =>
        MinamoMachineHelpers.FillDefaults(locals, callFun, ctx);

    private static bool TryCall(ExecutionContext ctx, int offset, ref MinamoObject? arg1, ref MinamoObject? arg2,
        ref MinamoNativeFunction function, ref MinamoObject[] locals, ref EvalStack evalStack)
    {
        if (ReferenceEquals(ctx.Error, MinamoFunction.CallbackPending))
        {
            ...
            function = (MinamoNativeFunction)ctx.CallBackFunction!;
    ...
    private static int ThrowIf(ExecutionContext ctx, int offset, ref MinamoNativeFunction function, ref MinamoObject[] locals, ref EvalStack evalStack)
    {
        var err = ctx.Error!;
        ...
        else
        {
            ...
            ctx.ErrorDump = null;
            ctx.Trace = null;
            throw new MinamoCodeException(err, cs, null);
        }
    }

    private static bool FindCatch(ExecutionContext ctx, ref MinamoNativeFunction function, ref MinamoObject[] locals, ref EvalStack evalStack, out int offset)
    {
    ...
    private static Stack<StackPoint> Dump(ExecutionContext ctx, int offset, MinamoNativeFunction function)
    {
    ...
    private static CallStackTrace BuildErrorTrace(ExecutionContext ctx, Stack<StackPoint> dump) =>
        ctx.Trace ?? new MinamoDebugger(ctx.RuntimeContext.Composition)
            .BuildCallStack(new Stack<StackPoint>(dump.Reverse()));

    private static MinamoObject AttachTrace(MinamoObject err, CallStackTrace? trace)
    {
        if (err is MinamoExceptionObject ex && ex.Trace is null && trace is not null)
        {
            ex.WithTrace(trace);
        }
    ...
    internal static (MinamoObject err, CallStackTrace? trace) GetErrorInformation(MinamoFunction func, Exception ex)
    {
        if (ex is MinamoCodeException err)
        {
        ...
        return (ErrorGenerators.RuntimeException(MinamoError.ExternalFunctionFailure, functionName, ex.Message), null);
    }
}

internal record struct RuntimeVar(string Name, MinamoObject Value);
