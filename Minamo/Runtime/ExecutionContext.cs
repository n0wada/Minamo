...
public sealed class ExecutionContext
{
    ...
    internal MinamoObject InvokeCallBackFunction()
    {
    ...
    internal MinamoObject InvokeCallBackFunction(MinamoObject arg)
    {
        var fn = CallBackFunction!;
        CallBackFunction = null;
        Error = null;
    ...
    internal MinamoObject InvokeCallBackFunction(MinamoObject arg1, MinamoObject arg2)
    {
        var fn = CallBackFunction!;
        CallBackFunction = null;
        Error = null;
    ...
    internal MinamoObject InvokeCallBackFunction(params MinamoObject[] args)
    {
        var fn = CallBackFunction!;
        CallBackFunction = null;
        Error = null;
    ...
    private MinamoObject? _error;
    public MinamoObject? Error
    {
        get => _error;
        internal set
        {
    ...
    public void ThrowIf()
    {
        if (Error is not null)
        {
            var err = Error;
            Error = null;
            throw new MinamoCodeException(err);
        }
    }
    #endregion
    ...
    {
        public MinamoObject[] Locals = null!;
        public MinamoObject[]? VarArgs;
        public int VarArgsSize;
        public int VarArgsIndex;
    }

    internal ArgContainer PushArguments(MinamoObject[] locals, int varArgsIndex, MinamoObject[]? varArgs = null)
    {
...
public enum MinamoExecutionLimitKind
{
    Instructions,
    Time,
    HostCommands,
...
public sealed class MinamoExecutionLimitException : MinamoRuntimeException
{
    public MinamoExecutionLimitException(MinamoExecutionLimitKind kind, string message)
        : base(message) => Kind = kind;

    public MinamoExecutionLimitKind Kind { get; }
}
...
internal sealed class ExecutionControl : IDisposable
{
    ...
    public void OnInstruction()
    {
        if (maxInstructions is not null && Instructions >= maxInstructions.Value)
        {
            throw new MinamoExecutionLimitException(
                MinamoExecutionLimitKind.Instructions,
    ...
    public void OnHostCommand()
    {
        ...
        {
            throw new MinamoExecutionLimitException(
                MinamoExecutionLimitKind.HostCommands,
...
internal sealed record VmSuspension(MinamoAwaitable? Awaitable = null);

internal sealed class ExecutionResult
{
    public long Ticks { get; }

    public MinamoObject? Value { get; }

    public ExecutionContext Context { get; }

    public TerminationReason Reason { get; }

    internal MinamoMachine.VmContinuation? Continuation { get; }
    ...
    private ExecutionResult(
        ...
        MinamoMachine.VmContinuation? continuation = null,
    ...
    internal static ExecutionResult Fetch(long ticks, MinamoObject? value, ExecutionContext ctx) =>
    ...
    internal static ExecutionResult Suspend(
        ExecutionContext ctx,
        MinamoMachine.VmContinuation continuation,
