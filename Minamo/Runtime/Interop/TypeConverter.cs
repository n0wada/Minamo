...
{
    public static MinamoObject ConvertFrom<T>(T obj) => ConvertFrom(obj, typeof(T), false);

    public static MinamoObject ConvertFrom(object? obj) =>
        ConvertFrom(obj, obj?.GetType()!, true);

    internal static MinamoObject ConvertFrom(object? obj, Type type) =>
        ConvertFrom(obj, type, false);

    private static MinamoObject ConvertFrom(object? obj, Type type, bool runtimeInterop)
    {
        if (obj is null)
        {
        ...
        switch (Type.GetTypeCode(type))
        {
            ...
            case TypeCode.Byte: return new MinamoInteger((byte)obj);
            case TypeCode.Int16: return new MinamoInteger((short)obj);
            case TypeCode.Int32: return new MinamoInteger((int)obj);
            case TypeCode.Int64: return new MinamoInteger((long)obj);
            case TypeCode.SByte: return new MinamoInteger((sbyte)obj);
            case TypeCode.UInt16: return new MinamoInteger((ushort)obj);
            case TypeCode.UInt32: return new MinamoInteger((uint)obj);
            case TypeCode.UInt64:
                var unsigned = (ulong)obj;
                return unsigned <= long.MaxValue
                    ? new MinamoInteger((long)unsigned)
                    : new MinamoInterop(type, obj);
            case TypeCode.String:
            case TypeCode.Char: return MinamoString.Get(obj.ToString());
            case TypeCode.Single: return new MinamoFloat((float)obj);
            case TypeCode.Double: return new MinamoFloat((double)obj);
            case TypeCode.Decimal: return new MinamoFloat((double)(decimal)obj);
            case TypeCode.Empty: return MinamoNil.Instance;
            default:
                ...
                else if (obj is IEnumerable seq)
                {
                    ...
                    var values = new List<MinamoObject>();
                    foreach (var item in seq)
                    {
                        values.Add(ConvertNested(item, elementType, runtimeInterop));
                    }

                    return new MinamoArray(values.ToArray());
                }
                else if (BCL.Type.IsAssignableFrom(type))
                {
                    return new MinamoInterop((Type)obj);
                }
                else
                {
                    return new MinamoInterop(type, obj);
    ...
    private static MinamoObject ConvertNested(object? value, Type declaredType, bool runtimeInterop) =>
        runtimeInterop
            ? ConvertFrom(value)
    ...
    public static T? ConvertTo<T>(ExecutionContext ctx, MinamoObject obj) => (T?)ConvertTo(ctx, obj, typeof(T));

    public static object? ConvertTo(ExecutionContext ctx, MinamoObject obj, Type type)
    {
    ...
    public static bool TryConvert(MinamoObject obj, Type type, out object? result)
    {
        result = default;
        long i8; double r8; string str;

        if (obj.TypeId == MinamoTypeCodes.Interop)
        {
            var interop = (MinamoInterop)obj;
        ...
        else if (BCL.MinamoObject.IsAssignableFrom(type))
        {
        ...
        switch (Type.GetTypeCode(type))
        {
            ...
            case TypeCode.Empty:
                ...
                return true;
            default:
                if (obj is MinamoDictionary map)
                {
                ...
                else if (obj is MinamoEnumerable enu)
                {
                    if (type.IsArray)
                    {
    ...
    private static bool TryGetFloat(MinamoObject obj, out double result)
    {
        if (obj is MinamoFloat f)
        {
            result = f.Value;
            return true;
        }
        else if (obj is MinamoInteger i)
        {
            result = i.Value;
            return true;
        }
        else if (obj is MinamoChar c)
        {
            result = c.Value;
            return true;
        }
    ...
    private static bool TryGetInteger(MinamoObject obj, out long result)
    {
        if (obj is MinamoInteger i)
        {
            result = i.Value;
            return true;
        }
        else if (obj is MinamoFloat f)
        {
    ...
    private static bool TryGetString(MinamoObject obj, out string result)
    {
        if (obj is MinamoString s)
        {
            result = s.Value;
            return true;
        }
    ...
    public static bool TryCreateTypedArray(MinamoObject[] arr, Type type, out Array? result)
    {
