...
[Generator]
public sealed class MinamoCommandGenerator : IIncrementalGenerator
{
    private const string ModuleAttribute = "Minamo.Hosting.MinamoModuleAttribute";
    private const string CommandAttribute = "Minamo.Hosting.MinamoCommandAttribute";
    private const string PropertyAttribute = "Minamo.Hosting.MinamoPropertyAttribute";
    private const string ForeignTypeAttribute = "Minamo.Hosting.MinamoForeignTypeAttribute";
    private const string CommandContext = "Minamo.Hosting.MinamoCommandContext";
    private const string ForeignType = "Minamo.Runtime.Types.MinamoForeignTypeInfo";
    private const string ForeignUnit = "Minamo.Linker.ForeignUnit";

    private static readonly DiagnosticDescriptor InvalidModule = Error(
        "MinamoH001", "Invalid host module", "MinamoModule on '{0}' requires a non-empty module name.");
    private static readonly DiagnosticDescriptor InvalidMethod = Error(
        "MinamoH002", "Invalid host command", "Method '{0}' cannot be exposed as a MinamoCommand: {1}");
    private static readonly DiagnosticDescriptor DuplicateCommand = Error(
        "MinamoH003", "Duplicate host command", "Module '{0}' contains more than one command named '{1}'.");
    private static readonly DiagnosticDescriptor InvalidParameter = Error(
        "MinamoH004", "Invalid host command parameter", "Parameter '{0}' on command '{1}' is not supported: {2}");
    private static readonly DiagnosticDescriptor InvalidForeignType = Error(
        "MinamoH005", "Invalid foreign host type", "Type '{0}' cannot be registered as a foreign host type: {1}");
    private static readonly DiagnosticDescriptor InvalidForeignUnit = Error(
        "MinamoH006", "Invalid foreign host unit", "Module unit '{0}' is invalid: {1}");
    private static readonly DiagnosticDescriptor InvalidProperty = Error(
        "MinamoH007", "Invalid host property", "Property '{0}' cannot be exposed as a MinamoProperty: {1}");
    ...
    private static void Emit(SourceProductionContext context, INamedTypeSymbol moduleType)
    {
        ...
        writer.Block($"public static class {extensionName}", typeWriter =>
        {
            ...
            {
                typeWriter.Block(
                    $"public static global::Minamo.Hosting.MinamoHost AddModule(this global::Minamo.Hosting.MinamoHost host, {TypeName(moduleType)} instance)",
                    methodWriter =>
                    {
            ...
            {
                typeWriter.Block(
                    $"public static global::Minamo.Hosting.MinamoHost {GetStaticModuleRegistrationName(moduleType)}(this global::Minamo.Hosting.MinamoHost host)",
                    methodWriter =>
                    {
                        EmitRegistration(
                            methodWriter,
        ...
        context.AddSource($"{GetHintName(moduleType)}.MinamoCommands.generated.cs", writer.ToSourceText());
    }
        ...
        foreach (var attribute in moduleType.GetAttributes().Where(IsForeignTypeAttribute))
        {
            ...
            if (type is null || type.IsAbstract || !InheritsFrom(type, ForeignType))
            {
                Report(context, InvalidForeignType, moduleType, type?.ToDisplayString() ?? "<unknown>",
                    "the type must be a non-abstract MinamoForeignTypeInfo");
                return null!;
            }
            ...
            if (IsCommandContext(parameter.Type))
            {
                ...
                if (contextCount > 1 || parameter.HasExplicitDefaultValue)
                {
                    Report(context, InvalidParameter, parameter, parameter.Name, method.Name,
                        "only one non-optional MinamoCommandContext parameter is allowed");
                    return false;
                }
            }
        ...
        string instance = null)
    {
        ...
        {
            writer.AppendLine(
                $"return global::Minamo.Hosting.MinamoCommandConvert.FromAwaitable({invocation});");
        }
        else if (method.ReturnsVoid)
        {
            writer.AppendLine(invocation + ";");
            writer.AppendLine("return global::Minamo.Runtime.Types.MinamoNil.Instance;");
        }
        else if (ReturnsDirectMinamoObject(method.ReturnType))
        {
            writer.AppendLine($"return {invocation} ?? global::Minamo.Runtime.Types.MinamoNil.Instance;");
        }
        else
        {
        ...
        for (var i = 0; i < exposedParameters.Length; i++)
        {
            ...
            var metadata = parameter.HasExplicitDefaultValue
                ? $"global::Minamo.Hosting.MinamoCommandParameter.Optional<{TypeName(parameter.Type)}>({Literal(parameter.Name)}, {DefaultValue(parameter)})"
                : $"global::Minamo.Hosting.MinamoCommandParameter.Required<{TypeName(parameter.Type)}>({Literal(parameter.Name)})";
        ...
        string instance)
    {
        ...
        writer.EndBlock(",");
        writer.AppendLine(
            $"global::Minamo.Hosting.MinamoCommandParameter.Required<{TypeName(symbol.Type)}>(\"value\"));");
    }
    ...
    private static string ArgumentConversion(ITypeSymbol type, int index)
    {
        ...
        if (nullableValueType is not null)
        {
            ...
            return $"{rawArgument} is global::Minamo.Runtime.Types.MinamoNil "
    ...
    private static string ArgumentConversion(
        ...
        string context)
    {
        if (ReturnsDirectMinamoObject(type))
        {
            return type.SpecialType == SpecialType.None && TypeName(type) != "global::Minamo.Runtime.Types.MinamoObject"
        ...
        {
            SpecialType.System_Object => $"global::Minamo.Hosting.MinamoCommandConvert.ToObject({context}, {rawArgument})",
            SpecialType.System_String => $"global::Minamo.Hosting.MinamoCommandConvert.ToString({context}, {rawArgument})",
            SpecialType.System_Boolean => $"global::Minamo.Hosting.MinamoCommandConvert.ToBoolean({context}, {rawArgument})",
            SpecialType.System_Byte => $"global::Minamo.Hosting.MinamoCommandConvert.ToByte({context}, {rawArgument})",
            SpecialType.System_Int16 => $"global::Minamo.Hosting.MinamoCommandConvert.ToInt16({context}, {rawArgument})",
            SpecialType.System_Int32 => $"global::Minamo.Hosting.MinamoCommandConvert.ToInt32({context}, {rawArgument})",
            SpecialType.System_Int64 => $"global::Minamo.Hosting.MinamoCommandConvert.ToInt64({context}, {rawArgument})",
            SpecialType.System_SByte => $"global::Minamo.Hosting.MinamoCommandConvert.ToSByte({context}, {rawArgument})",
            SpecialType.System_UInt16 => $"global::Minamo.Hosting.MinamoCommandConvert.ToUInt16({context}, {rawArgument})",
            SpecialType.System_UInt32 => $"global::Minamo.Hosting.MinamoCommandConvert.ToUInt32({context}, {rawArgument})",
            SpecialType.System_UInt64 => $"global::Minamo.Hosting.MinamoCommandConvert.ToUInt64({context}, {rawArgument})",
            SpecialType.System_Single => $"global::Minamo.Hosting.MinamoCommandConvert.ToSingle({context}, {rawArgument})",
            SpecialType.System_Double => $"global::Minamo.Hosting.MinamoCommandConvert.ToDouble({context}, {rawArgument})",
            SpecialType.System_Decimal => $"global::Minamo.Hosting.MinamoCommandConvert.ToDecimal({context}, {rawArgument})",
            SpecialType.System_Char => $"global::Minamo.Hosting.MinamoCommandConvert.ToChar({context}, {rawArgument})",
            _ => $"global::Minamo.Hosting.MinamoCommandConvert.ToObject<{TypeName(type)}>({context}, {rawArgument})"
        };
    }
    ...
    private static string ReturnConversion(ITypeSymbol type, string value)
    {
        if (type.IsReferenceType && type.NullableAnnotation == NullableAnnotation.Annotated)
        {
            ...
            return $"{value} is null ? global::Minamo.Runtime.Types.MinamoNil.Instance : {direct}";
        }
        ...
        if (target is not null)
        {
            ...
            return $"{value}.HasValue ? {direct} : global::Minamo.Runtime.Types.MinamoNil.Instance";
        }

        return type.SpecialType switch
        {
            SpecialType.System_String => $"global::Minamo.Hosting.MinamoCommandConvert.FromString({value})",
            SpecialType.System_Boolean => $"global::Minamo.Hosting.MinamoCommandConvert.FromBoolean({value})",
            SpecialType.System_Byte => $"global::Minamo.Hosting.MinamoCommandConvert.FromInteger({value})",
            SpecialType.System_Int16 => $"global::Minamo.Hosting.MinamoCommandConvert.FromInteger({value})",
            SpecialType.System_Int32 => $"global::Minamo.Hosting.MinamoCommandConvert.FromInteger({value})",
            SpecialType.System_Int64 => $"global::Minamo.Hosting.MinamoCommandConvert.FromInteger({value})",
            SpecialType.System_SByte => $"global::Minamo.Hosting.MinamoCommandConvert.FromInteger({value})",
            SpecialType.System_UInt16 => $"global::Minamo.Hosting.MinamoCommandConvert.FromInteger({value})",
            SpecialType.System_UInt32 => $"global::Minamo.Hosting.MinamoCommandConvert.FromInteger({value})",
            SpecialType.System_UInt64 => $"global::Minamo.Hosting.MinamoCommandConvert.FromObject<ulong>({value})",
            SpecialType.System_Single => $"global::Minamo.Hosting.MinamoCommandConvert.FromFloat({value})",
            SpecialType.System_Double => $"global::Minamo.Hosting.MinamoCommandConvert.FromFloat({value})",
            SpecialType.System_Decimal => $"global::Minamo.Hosting.MinamoCommandConvert.FromFloat((double){value})",
            SpecialType.System_Char => $"global::Minamo.Hosting.MinamoCommandConvert.FromChar({value})",
            _ => $"global::Minamo.Hosting.MinamoCommandConvert.FromObject<{TypeName(type)}>({value})"
        };
    }

    private static bool ReturnsDirectMinamoObject(ITypeSymbol type) =>
        TypeName(type) == "global::Minamo.Runtime.Types.MinamoObject"
        || type is INamedTypeSymbol namedType && InheritsFrom(namedType, "Minamo.Runtime.Types.MinamoObject");
    ...
    private static DiagnosticDescriptor Error(string id, string title, string message) =>
        new(id, title, message, "Minamo.Hosting", DiagnosticSeverity.Error, isEnabledByDefault: true);
