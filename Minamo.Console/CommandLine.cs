...
internal static class CommandLine
{
    ...
    {
        public abstract MinamoObject ToMinamoObject();
    }
    ...
    private sealed class ScalarValue(string? value) : OptionValue
    {
        ...
        public override MinamoObject ToMinamoObject() => Value is null ? MinamoBool.True : new MinamoString(Value);
    }
    ...
    private sealed class ArrayValue(IEnumerable<string?> values) : OptionValue
    {
        ...
        public override MinamoObject ToMinamoObject() =>
            new MinamoTuple(Values.Select(value =>
                (MinamoObject)(value is null ? MinamoBool.True : new MinamoString(value))).ToArray());
    }
    ...
    public static CommandLineOptions Read(string[] args)
    {
        ...
        if (values.Count > 0)
        {
            var userArguments = new List<MinamoObject>();
            ...
            foreach (var (name, value) in values)
            {
                ...
                userArguments.Add(new MinamoLabel(name.TrimStart('-'), value.ToMinamoObject()));
            }

            options.UserArguments = new MinamoTuple(userArguments.ToArray());
        }
...
internal sealed class CommandLineOptions
{
    ...
    [Binding(Help = "A .nami file to execute. Multiple files or directories are supported.", Category = Compiler)]
    ...
    public MinamoTuple? UserArguments { get; set; }
    ...
    public IEnumerable<string> GetFileNames()
    {
        foreach (var item in FileNames ?? Array.Empty<string>())
        {
            ...
            {
                foreach (var file in Directory.GetFiles(item, "*.nami").OrderBy(file => file))
                {
                    yield return file;
                }
            }
