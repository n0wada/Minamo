...
namespace Minamo.UnitTesting.Parser;
...
public sealed class ParserTests
{
    ...
    {
        var result = MinamoParser.Parse(source, "<removed-bitwise-operator>");
    ...
    {
        var result = MinamoParser.Parse(source, "<overloadable-shift-operator>");
    ...
    {
        var successful = MinamoParser.Parse("let value = 42");
        var failed = MinamoParser.Parse("let =", "generated.nami");

        Assert.True(successful.Success);
        Assert.False(failed.Success);
        Assert.All(failed.Errors, error => Assert.Equal("generated.nami", error.File));
    }

    [Fact]
    public void AllowsManyAsAnIdentifierWhileTypeRemainsReserved()
    {
        var many = MinamoParser.Parse("let many = 1\nmany");
        var type = MinamoParser.Parse("let type = 1");
    ...
    {
        var result = MinamoParser.Parse("let slash = '\\\\'");
    ...
    {
        var result = MinamoParser.Parse(source);
        var bareResult = MinamoParser.Parse(
    ...
    public void ParsesFileAndPreservesPath()
    {
        var directory = Path.Combine(
            Path.GetTempPath(),
            "minamo-parser-" + Guid.NewGuid().ToString("N"));
        var path = Path.Combine(directory, "invalid.nami");
        ...
        try
        {
            ...
            var result = MinamoParser.ParseFile(path);
    ...
    public async Task LoadsSourceBufferFromFileAsynchronously()
    {
        var directory = Path.Combine(
            Path.GetTempPath(),
            "minamo-parser-async-" + Guid.NewGuid().ToString("N"));
        var path = Path.Combine(directory, "source.nami");
        Directory.CreateDirectory(directory);
        ...
        try
        {
            ...
            var result = MinamoParser.Parse(buffer);
    ...
    public async Task CancelsAsynchronousSourceBufferRead()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => SourceBuffer.FromFileAsync("unused.nami", cancellation.Token));
    }

    [Fact]
    public void PreservesParameterizedTypeHintsAndExpandsNullableSyntax()
    {
        var result = MinamoParser.Parse(SourceBuffer.FromString(
    ...
    public void AcceptsNullableAndParameterizedHintsAcrossDeclarations()
    {
        ...
        var result = MinamoParser.Parse(SourceBuffer.FromString(source, "<type-hint-forms>"));
    ...
    {
        var result = MinamoParser.Parse(SourceBuffer.FromString(source, "<invalid-type-hints>"));
    ...
    {
        var result = MinamoParser.Parse(SourceBuffer.FromString(source, "<legacy-fields>"));
    ...
    {
        var result = MinamoParser.Parse(SourceBuffer.FromString(source, "<truncated>"));
