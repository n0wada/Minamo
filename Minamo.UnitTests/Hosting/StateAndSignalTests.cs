...
public sealed class StateAndSignalTests
{
    ...
    {
        using var instance = new MinamoHost(new()
            {
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new MinamoHost(new()
        {
            Signals = new() { MaxPending = 0 }
        }));
        Assert.Throws<ArgumentNullException>(() => new MinamoHost(new()
        {
            Signals = null!
        }));
    }
