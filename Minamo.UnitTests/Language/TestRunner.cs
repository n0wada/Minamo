...
internal sealed class TestRunner
{
    ...
    private static TestBlockInfo[] GatherTests(IEnumerable<string> files, List<BuildMessage> warns)
    {
        ...
        {
            var res = MinamoParser.Parse(SourceBuffer.FromFile(file));
        ...
        List<BuildMessage> warns)
    {
        ...
        Dictionary<string, MinamoCodeModel> inits;
        ...
        foreach (var bi in selectedBlocks)
        {
            ...
            if (inits.TryGetValue(fileName, out var init))
            {
                ...
                ast = new MinamoCodeModel(root, imports, ast.FileName);
            }
            ...
            var execution = session.ExecuteAsync(ast).GetAwaiter().GetResult();
            warns.AddRange(execution.Diagnostics
                .Where(diagnostic => diagnostic.Severity == MinamoDiagnosticSeverity.Warning)
            ...
            if (!execution.Success)
            {
                ...
                report.Results.Add(new TestResult
                {
                    ...
                    {
                        Kind: MinamoFailureKind.Limit,
                        Limit: MinamoExecutionLimitKind.Time
                    },
