...
public sealed class TestDiagnosticsTests
{
    ...
    {
        var report = new TestReport { TestFiles = ["sample.nami"] };
        report.Results.Add(new TestResult
        {
            Name = "failure",
            FileName = "sample.nami",
            Error = "Assertion failed.",
            StackTrace = "Minamo.Runtime.MinamoRuntimeException: Assertion failed.",
            Expected = "1",
            Actual = "2",
        ...
        Assert.Contains("File: sample.nami", text);
    ...
    private sealed class TemporaryLanguageTest : IDisposable
    {
        ...
        public static TemporaryLanguageTest Create(string source)
        {
            var path = System.IO.Path.Combine(
                ...
                $"minamo-language-{Guid.NewGuid():N}.nami");
